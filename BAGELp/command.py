from BAGELp.formatInput import Bagel
import sys
import os
import argparse
...
version = pkg_resources.require("BAGELp")[0].version


def main():  # pragma: no cover
    usage = "\n %prog [options] -f counts.tsv -l library.tsv"

    optParser = argparse.ArgumentParser(prog='BAGELp',
    ...
    required.add_argument("-i", "--fc_file", type=str, dest="fc_file", required=True,
                          default="", help="Tab-delmited file of sgRNA and fold changes")
    ...
    log.info(opts)
    myBagel = Bagel(**vars(opts))
    myBagel.run_analysis()
